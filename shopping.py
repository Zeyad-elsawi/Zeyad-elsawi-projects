...
def load_data(filename):
    ...
    """
    csv_file=pandas.read_csv('shopping.csv')
    evidence=[]
    label=[]
    ...
    VisitorType={'Returning_Visitor':1,
                 'New_Visitor':0}
    evidence_df = evidence_df.replace(VisitorType)
    Weekend = {'TRUE': 1,
                   'FALSE': 0}
    evidence_df = evidence_df.replace(Weekend)
    Revenue = {'TRUE': 1,
               'FALSE': 0}
    labels_df=labels_df.replace(Revenue)
...
def train_model(evidence, labels):
    ...
    """
    eigh = KNeighborsClassifier(n_neighbors=1)
